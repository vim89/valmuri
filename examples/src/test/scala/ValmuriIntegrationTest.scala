...
class ValmuriIntegrationTest extends FunSuite {
  ...
  test("web server should actually start and respond to /") {
    assume(!isCI, "Skipping server-alive test in CI environment")
    ...
    // Verify the server socket is listening before sending HTTP requests
    val socketAlive =
      try {
        val s = new Socket("localhost", testPort)
        s.close()
        true
      } catch {
        case _: Exception => false
      }
    assert(socketAlive, s"Server on port $testPort is not accepting connections")
